...
model_name = 'ResNet_base_nopretraining_syntheticft'
...
log(f'Model: This is ResNet not {model_name}')
log(f'Device: {device}')
...
model.load_state_dict(torch.load(f'{PATH}/ResNet_base_nopretraining.pth'))
model.to(device)
...
for epoch in range(0, NUM_EPOCHS, 1): # evaluate every epoch
    ...
    if val_loss < min_loss:
        log(f'[INFO] val_loss has been improved from {min_loss:.5f} to {val_loss:.5f}. Saving Model!')
        min_loss = val_loss
...
log("{time_duration} takes for training {model_name} !")
