...
# 2023-12-13 : Make synthetic base model with only training synthetic
# Run Model 2

# Model : Deit
# Model 1 (Base) : pneumonia MNIST => test with pneumonia MNIST
# Model 1-2 (Synthetic base) : synthetic => test with pneumonia MNIST
...
for epoch in range(0, NUM_EPOCHS, 1): # evaluate every epoch
    train_loss, train_acc = model_train(model, train_loader_synthetic, loss, optimizer, device)
    val_loss, val_acc = model_evaluate(model, val_loader_synthetic, loss, device)
...
# pth 불러오기
model.load_state_dict(torch.load(f'{PATH}/DeiT_synthetic.pth'))
model.to(device)
#model.eval()
