...
model_name = 'ResNet_synthetic_chestMNIST'
...
# Dataloaders
train_loader = DataLoader(train_dataset_synthetic, batch_size=BATCH_SIZE, shuffle=True)
val_loader = DataLoader(val_dataset_synthetic, batch_size=BATCH_SIZE, shuffle=False)
...
#        lbl = lbl.type(torch.FloatTensor)
        ...
        # output[0] : torch.Size([128, 2]) (BATCH_SIZE, NUM_CLASSES), lbl : torch.Size([128]) (BATCH_SIZE)
#        print("Check shape of loss ==>", output[0].shape, lbl.squeeze(dim=-1).shape) #torch.Size([128, 2]) torch.Size([128])
#        print(output[0],lbl.squeeze(dim=-1) )
#        print("loss ==>",loss)
        loss.backward()
        optimizer.step()
        # scheduler.step()
...
model.fc= nn.Linear(in_features, 2)
# pth 불러오기
# model.load_state_dict(torch.load(f'{PATH}/ResNet_base.pth'))
